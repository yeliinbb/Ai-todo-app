...
const CommonChatSystemButton: React.FC<CommonChatSystemButtonProps> = ({ onClick, disabled = false, children }) => {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="bg-system-white border border-solid border-gray400 backdrop-blur-md text-gray-600 font-semibold text-sm pr-3 pl-5 py-1.5 rounded-full w-full flex justify-between items-center cursor-pointer"
    >
      <span>{children}</span>
