...
const CommonChatSystemButton: React.FC<CommonChatSystemButtonProps> = ({ onClick, disabled = false, children }) => {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="bg-grayTrans-20060 backdrop-blur-3xl text-gray-600 font-semibold text-sm pr-3 pl-5 py-2 rounded-full w-full flex justify-between items-center"
    >
      <span>{children}</span>
